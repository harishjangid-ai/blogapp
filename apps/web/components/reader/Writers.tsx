  ...
  return (
    <>
      <div className="flex flex-col gap-3">
        ...
        <Input onChange={(e) => setSearch(e.target.value)} placeholder="Search writer by name, content, proffession..." className="w-75"/>
        <div className="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-2">
          {filteredWriters?.map((data) => (
            <div className="flex flex-col border rounded-2xl p-6 items-center w-full text-center gap-3 text-gray-500 font-thin" key={data._id}>
              <div className="w-20 h-20 rounded-full bg-linear-to-br from-purple-500 to-indigo-500 flex items-center justify-center text-white font-bold text-xl">
                {data.fullName.split(" ").map(w=> w[0].toUpperCase()).join("")}
              </div>
              <div>
              ...
              <div className="">
                <p>{data.profession} || {data.contentType.charAt(0).toUpperCase() + data.contentType.slice(1)}</p>
                <p className="text-sm wrap-break-word line-clamp-2">{data.description}</p>
              </div>
            </div>
          ))}
        </div>
